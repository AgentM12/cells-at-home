...
Expected Protocol:
- a client-side log file (default: latest.log)
- Lines output to chat using tellraw to that player.
- Lines should follow "[DEBUG/ID] data" where ID can be STATS_CONFIG, STATS_START, STATS, STATS_END, FORMAT_CONFIG, FORMAT_START, FORMAT_TICK, FORMAT_END
- First, one should post the formats of the 4 stats events with space separated values.
- Then one should log the config, the start, then all the stats calls and end off with the end.
- The program expects a PROFILER_ID which is the id of the current run, such that runs can be separated. it should be the first number in the config, note that the format for config should then start with PROFILER_ID or something similar.
- Will output a csv file with the FORMATs as headers and the STATSs as values.
'''
